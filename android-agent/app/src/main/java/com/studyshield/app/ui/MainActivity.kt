...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildUI() {
        ...
        root.addView(TextView(this).apply {
            ...
            setTextColor(0xFF0EA5E9.toInt()); gravity = Gravity.CENTER
            setPadding(0, 0, 0, 48)
        })
        ...
        card.addView(TextView(this).apply {
            text = "DEVICE ID"; textSize = 11f; setTextColor(0xFF94A3B8.toInt()); letterSpacing = 0.12f
        })
        card.addView(TextView(this).apply {
            text = deviceId; textSize = 28f; setTextColor(0xFF22C55E.toInt())
            typeface = Typeface.MONOSPACE; setPadding(0, 10, 0, 10)
        })
        card.addView(TextView(this).apply {
            text = "Enter this in the parent app when adding this device."
            textSize = 12f; setTextColor(0xFF64748B.toInt()); lineSpacingExtra = 4f
        })
        root.addView(card)

        // Copy button
        val btnLp = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT)
        btnLp.bottomMargin = 16
        root.addView(Button(this).apply {
            text = "📋  Copy Device ID"; textSize = 15f; isAllCaps = false
